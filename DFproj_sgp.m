...
function [x, biter, siter,r] = DFproj_sgp(b, c, dia)
...
% and Cavicchioli available at: http://www.unife.it/prisma/software and it
% is used by the SGP for the projection of points onto the feasible set if
% the problem is subject to the total flux conservation constraint.
...
% biter = integer, total number of backtracking iterations;
% siter = integer, total number of secant iterations;
% r     = double, linear constraint residual value, i.e.,
%                   r = (sum(sum(x))-b).
%
%==========================================================================
% 
...
% Bracketing Phase
x = max(0,(c+lambda)./dia); r = sum(sum(x)) - b;
